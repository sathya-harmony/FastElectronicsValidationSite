...
function AIInsights({ authToken }: { authToken: string | null }) {
  const { data: insightsData, isLoading } = useQuery({
    queryKey: ["ai-insights", authToken],
    queryFn: async () => {
  ...
  if (!insightsData?.insights || insightsData.insights.length === 0) {
    return <p className="text-sm text-muted-foreground">Gathering more data for analysis...</p>;
  }
    ...
    <ul className="space-y-2">
      {insightsData.insights.map((insight: string, idx: number) => (
        <motion.li
          ...
          className="text-sm flex gap-2 items-start text-slate-700"
        >
          <span className="mt-1 block h-1.5 w-1.5 rounded-full bg-violet-500 shrink-0" />
          {insight}
        </motion.li>
      ))}
    </ul>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <motion.div variants={fadeInUp} className="mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <Card className="border-black/5 premium-shadow">
                <CardHeader>
                  <CardTitle className="text-sm font-medium text-muted-foreground">Potential Gross Margin</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-3xl font-bold text-emerald-600">
                    {offers.length > 0 ? (
                      (() => {
                        const totalBase = offers.reduce((sum: number, o: any) => sum + Number(o.basePrice), 0);
                        const totalSelling = offers.reduce((sum: number, o: any) => sum + Number(o.price), 0);
                        const margin = ((totalSelling - totalBase) / totalSelling) * 100;
                        return `${margin.toFixed(1)}%`;
                      })()
                    ) : "0%"}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">Avg. margin across catalog</p>
                </CardContent>
              </Card>

              <Card className="border-black/5 premium-shadow">
                <CardHeader>
                  <CardTitle className="text-sm font-medium text-muted-foreground">Avg. Order Value (Est.)</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-3xl font-bold">
                    ₹{offers.length > 0 ? Math.round(offers.reduce((sum: number, o: any) => sum + Number(o.price), 0) / offers.length).toLocaleString() : 0}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">Based on single item cart</p>
                </CardContent>
              </Card>

              <Card className="border-black/5 premium-shadow">
                <CardHeader>
                  <CardTitle className="text-sm font-medium text-muted-foreground">Delivery Economics</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="text-3xl font-bold text-blue-600">
                    ₹{stores.length > 0 ? Math.round(50 + (12.75 * (stores.reduce((sum: number, s: any) => sum + Number(s.distanceKm), 0) / stores.length))) : 0}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">Avg. delivery cost per order</p>
                </CardContent>
              </Card>
            </div>
          </motion.div>
